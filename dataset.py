...
def _read_file(filename):
    ...
    i = 1
    train = True
    # if filename.find("train") != -1: train = True
    ...
    with open(filename, 'r') as fp:
        ...
        for row in reader:
            if i == 1:
                i = 0
                continue
            if 0 > int(row[0]) or int(row[0]) > 10:
                continue
            ids.append(row[0])
...
class DialogueDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filename, history_len=50, response_len=15, vocab=None,
                 update_vocab=True):
        """
    ...
    def get_input_features(self, history):
        ...
        return torch.from_numpy(h_seq).unsqueeze(0), torch.from_numpy(
            h_pos).unsqueeze(0), torch.from_numpy(h_seg).unsqueeze(0)
